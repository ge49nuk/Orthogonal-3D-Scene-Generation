...
def visualize_depth_u8(depth, minmax=None):
    """
    depth: (H, W)
    """
    x = np.nan_to_num(depth)  # change nan to 0
...
def repaint(args, image_pil):
    model_path = os.path.join(args.models_path, "stable-diffusion-2-depth")
    if not os.path.exists(model_path):
    ...
    image = pipe(prompt=args.prompt, image=image_pil, strength=0.7).images[0]
    return image
